...
import { Play, Pause, Download, Volume2, VolumeX, Loader2, Camera, X, ChevronLeft, ChevronRight } from 'lucide-react'
...
export function StoryGenerator({ isOpen, onClose, flyer }: StoryGeneratorProps) {
  ...
  const slides = [
    ...
    ...flyer.products_data.map(p => ({ type: 'product', product: p })),
  ...
  }, [isPlaying, currentSlide, progress])
  ...
  const exportAsImages = async () => {
    ...
    try {
      ...
      link.download = `story-${flyer.title}-${currentSlide}.jpg`
      link.href = images[currentSlide]
      link.click()
      
      toast.success('Imagens geradas! Você pode baixar cada slide.')
    } catch (err) {
      console.error(err)
        ...
        <div className="flex h-[85vh] flex-col md:flex-row">
            ...
            >
              ...
              <div 
                ...
                style={{
                  ...
                  backgroundImage: flyer.config?.backgroundType === 'image' ? `url(${flyer.config.backgroundUrl})` : 'none',
              ...
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-8 text-center">
                ...
                {currentSlideData.type === 'product' && currentSlideData.product && (
          ...
          <div className="w-full md:w-80 bg-zinc-950 p-6 flex flex-col gap-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                {isPlaying ? <><Pause className="mr-2 h-4 w-4" /> Pausar</> : <><Play className="mr-2 h-4 w-4" /> Play</>}
              </Button>
              <Button 
                variant="outline"
            ...
            <div className="mt-auto">
              ...
              <Progress value={((currentSlide + 1) / slides.length) * 100} className="h-1 bg-zinc-800" />
            </div>

            <Button 
              variant="ghost" 
              className="text-zinc-500 hover:text-white hover:bg-zinc-900 h-10 font-bold uppercase text-[10px]"
              onClick={onClose}
            >
              <X className="mr-2 h-4 w-4" /> Fechar Visualizador
            </Button>
          </div>
        </div>
      </DialogContent>
