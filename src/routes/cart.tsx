  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (currentSession) {
        ...
        const { data: addrData } = await supabase.from('user_addresses').select('*').eq('user_id', currentSession.user.id).order('created_at', { ascending: false });
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const userSummary = formatWhatsAppMessage('order_summary', {
        id: order.id, customer_name: customerName, address: addressStr,
        payment_method: paymentMethod === 'money' ? `Dinheiro${changeFor ? ` (Troco para R$ ${changeFor})` : ' (Sem troco)'}` : paymentMethod,
        items, subtotal: total, discount, delivery_fee: deliveryFee, total_amount: total - discount + deliveryFee
      }, templates);
      await sendWhatsAppMessage(customerPhone, `✅ *PEDIDO RECEBIDO COM SUCESSO!* ✅\n\nOlá *${customerName}*, seu pedido foi processado e já estamos preparando tudo!\n\n${userSummary}`);
          ...
          <>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center bg-gray-100 rounded-lg p-1">
                        <button onClick={() => updateQuantity(item.id, item.quantity - (item.is_weight_based ? 0.1 : 1))} className="p-1 text-green-600 hover:bg-white rounded-md transition-colors"><Minus size={16} /></button>
                        <div className="flex flex-col items-center px-2 min-w-[40px]">
                          <span className="text-xs font-bold leading-none">{item.is_weight_based ? item.quantity.toFixed(3) : item.quantity}</span>
                          <span className="text-[7px] font-black uppercase opacity-40 leading-none mt-0.5">{item.is_weight_based ? 'kg' : (item.unit || 'un')}</span>
                        </div>
                        <button onClick={() => updateQuantity(item.id, item.quantity + (item.is_weight_based ? 0.1 : 1))} className="p-1 text-green-600 hover:bg-white rounded-md transition-colors"><Plus size={16} /></button>
                      </div>
                      <button onClick={() => removeFromCart(item.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={18} /></button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="bg-white rounded-3xl shadow-sm border p-6 space-y-4">
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <input type="text" placeholder="Digite seu cupom" value={coupon} onChange={(e) => setCoupon(e.target.value)} className="w-full h-12 bg-zinc-50 border-zinc-100 rounded-xl px-4 font-bold text-sm focus:bg-white focus:ring-2 focus:ring-green-500 transition-all uppercase" />
                  {discount > 0 && (
                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                      <div className="w-5 h-5 bg-green-500 rounded-full flex items-center justify-center"><Ticket size={12} className="text-white" /></div>
                    </div>
                  )}
                </div>
                <Button onClick={handleApplyCoupon} disabled={isCheckingCoupon || !coupon} className="h-12 px-6 rounded-xl font-black uppercase tracking-widest text-[10px] bg-zinc-900 text-white">
            ...
            {!session && (
              <div className="bg-white rounded-3xl shadow-xl border-4 border-primary/20 p-8 space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
                <div className="text-center space-y-2">
                  <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-4"><User className="text-primary" size={32} /></div>
              ...
              {addresses.length === 0 ? (
                ...
                <div className="bg-white rounded-3xl shadow-sm border p-4 space-y-3">
                  <div className="flex items-start gap-3">
                    <div className="p-2 bg-green-50 text-green-600 rounded-xl"><MapPin size={20} /></div>
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500">{selectedAddress?.street}, {selectedAddress?.number}</p>
                      <p className="text-xs text-gray-400">{selectedAddress?.neighborhood} - {selectedAddress?.city}</p>
                      ...
                      {isValidDeliveryArea === false && (
                        <div className="mt-2 text-[10px] text-red-600 bg-red-50 p-2 rounded-lg border border-red-100 flex items-center gap-2">
                          <AlertCircle size={14} />
                          <span className="font-bold uppercase">Área não atendida:</span> Não entregamos em {selectedAddress.neighborhood}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div key={method.id} className="space-y-2">
                    ...
                    {method.id === 'money' && paymentMethod === 'money' && (
                      <div className="px-4 py-3 bg-white border-x-2 border-b-2 border-green-600/20 rounded-b-2xl -mt-2 space-y-2 animate-in slide-in-from-top-2">
                        ...
                        <div className="flex gap-2">
                          <input type="number" placeholder="Valor em R$ (ex: 50.00)" value={changeFor} onChange={(e) => setChangeFor(e.target.value)} className="flex-1 h-10 bg-zinc-50 border-zinc-100 rounded-lg px-3 text-sm font-bold focus:ring-2 focus:ring-green-500" />
            ...
            <div className="bg-white rounded-3xl shadow-sm border p-6 space-y-4">
              ...
              <div className="flex justify-between items-center text-gray-500">
                <span className="flex items-center gap-1">Entrega <span className="text-[10px] font-bold text-zinc-400">({selectedAddress?.neighborhood || 'Escolha o bairro'})</span></span>
                {isValidDeliveryArea === false ? (
