...
async function migrate() {
  ...
  for (const prod of products) {
    ...
    sql += `INSERT INTO public.products (${columns.join(', ')}) VALUES (${values.join(', ')}) ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, price = EXCLUDED.price, stock = EXCLUDED.stock, category_id = EXCLUDED.category_id;\n`;
  }
